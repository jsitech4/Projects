...
.tank-wrap{display:flex;align-items:center;justify-content:center;gap:28px;width:100%;padding:8px 0 14px}
.tank{position:relative;width:170px;height:250px;border:4px solid var(--text);border-radius:18px 18px 28px 28px;background:rgba(148,163,184,.10);overflow:hidden;box-shadow:inset 0 0 0 2px rgba(255,255,255,.35)}
.tank:before{content:'';position:absolute;left:24px;right:24px;top:12px;height:7px;border:2px solid var(--text);border-radius:8px}
.liquid{position:absolute;left:0;right:0;bottom:0;height:0%;background:linear-gradient(180deg,#38bdf8,#0284c7);transition:height .5s ease}
.liquid:before{content:'';position:absolute;left:-10%;top:-8px;width:120%;height:16px;border-radius:50%;background:rgba(186,230,253,.72)}
.tank-label{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;z-index:1;font-size:27px;font-weight:800;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,.5)}
.valve{display:flex;flex-direction:column;align-items:center;gap:10px;min-width:150px}
.valve-body{width:74px;height:74px;border:5px solid var(--text);transform:rotate(45deg);background:var(--bad);transition:background .25s ease}
.valve-body.open{background:var(--good)}
.valve-stem{width:8px;height:38px;background:var(--text);margin-top:-32px;z-index:1}
.valve-title{font-size:18px;font-weight:800}.valve-state{font-size:13px;color:var(--muted)}
...
@media(max-width:900px){
  ...
  .metric-row{flex-direction:column;text-align:center}
...
<div class="header">
  <div class="title">
    <h1>Monitoring System for the supervison of water level and temperature in oil-water seperation vessels</h1>
    <p>Ogbonna Enyichukwu Tobechi || 23310211578.</p>
  </div>
...
<div class="card span12">
  ...
  <div class="tank-wrap">
    ...
    <div class="valve">
      <div class="valve-body" id="valveBody"></div>
      <div class="valve-stem"></div>
      <div class="valve-title">Outlet Valve</div>
      <div class="valve-state" id="valveState">Relay OFF</div>
    </div>
  </div>

  <div class="bar">
    <div class="fill good" id="tankFill"></div>
  </div>
...
<div class="card span12">
  ...
  <div class="metric-row">
    <strong>Full level threshold (%)</strong>
    <span>
      <input
        id="fullThreshold"
        type="number"
  ...
  <div class="metric-row">
    <strong>Low level threshold (%)</strong>
    <span>
      <input
        id="lowThreshold"
        type="number"
  ...
  <div class="metric-row">
    <strong>Relay latch trigger</strong>
    <span>
      <select
        id="latchMode"
        onchange="markSettingsDirty()">
        <option value="0">Disabled</option>
        <option value="1">Latch at full level</option>
        <option value="2">Latch at low level</option>
      </select>
    </span>
  </div>

  <div class="actions">
    <button class="btn good" onclick="saveSettings()">Save settings</button>
    <span class="small" id="settingsMessage">
      Saved settings load automatically at boot.
    </span>
  </div>
</div>
...
<div class="card span12">
  <div class="label">Monitoring Data Log</div>

  <pre id="readingLog">Loading...</pre>
  ...
  <div class="actions">
    ...
    <button class="btn" onclick="location.href='/download/reading_log.csv'">Download Reading CSV</button>
...
<div class="card span6">
  <div class="label">Recent Analysis Log</div>
  <pre id="analysisLog">Loading...</pre>
</div>

<div class="card span6">
  <div class="label">Recent Event Log</div>
...
function setFill(id, v, reverse)
{
  ...
  if (reverse)
    cls = v >= 75 ? 'good' : v >= 50 ? 'warn' : 'bad';
  else
    cls = v >= 75 ? 'bad' : v >= 50 ? 'warn' : 'good';
...
async function loadStatus()
{
  try
  {
    ...
    document.getElementById('temp').innerHTML =
      s.temp_valid
        ? fmt(s.temperature_c, 1) + ' <span class="unit">°C</span>'
        : 'N/A';

    document.getElementById('distance').innerHTML =
      s.level_valid
        ? fmt(s.distance_cm, 1) + ' <span class="unit">cm</span>'
        : 'N/A';

    document.getElementById('levelPercent').innerHTML =
      s.level_valid
        ? fmt(s.level_percent, 0) + ' <span class="unit">%</span>'
        : 'N/A';

    document.getElementById('liquidFill').style.height =
      (s.level_valid
        ? clamp(s.level_percent, 0, 100)
        : 0) + '%';

    document.getElementById('tankFill').style.width =
      (s.level_valid
        ? clamp(s.level_percent, 0, 100)
        : 0) + '%';

    document.getElementById('tankLabel').textContent =
      s.level_valid
        ? fmt(s.level_percent, 0) + '%'
        : 'N/A';

    document.getElementById('backend').textContent =
      'Backend: ' + s.backend +
      ' | Internal: ' +
      (s.internal_ready ? 'ready' : 'not ready');

    document.getElementById('backendPill').textContent =
      'Backend: ' + s.backend;

    document.getElementById('sensorStatus').textContent =
      (s.temp_valid && s.level_valid)
        ? 'READY'
        : 'WAITING';

    document.getElementById('sensorStatusText').textContent =
      (s.temp_valid && s.level_valid)
        ? 'PT100 and ultrasonic online'
    ...
    document.getElementById('recommendation').textContent =
      'Live tank measurements are available.';
    ...
    document.getElementById('valveBody').className =
      'valve-body' +
      (s.relay_on ? ' open' : '');

    points.push({
      temp: Number(s.temperature_c) || 0,
      level: Number(s.level_percent) || 0
    });
    ...
    if (!settingsDirty)
    {
      document.getElementById('fullThreshold').value =
        s.full_level_percent;

      document.getElementById('lowThreshold').value =
        s.low_level_percent;

      document.getElementById('latchMode').value =
        s.relay_latch_mode;
    }
  }
  catch (e)
...
{
  const full =
    Number(document.getElementById('fullThreshold').value);

  const low =
    Number(document.getElementById('lowThreshold').value);
  ...
  try
  {
    const response =
      await fetch(
        '/api/settings/save?full=' +
        encodeURIComponent(full) +
        '&low=' +
        encodeURIComponent(low) +
        '&mode=' +
    ...
    if (result.ok)
    {
      ...
      message.textContent =
        'Settings saved to internal flash.';
    }
    else
    {
      message.textContent =
...
async function loadLogs()
{
  try
  {
    ...
    document.getElementById('readingLog').textContent =
      s.reading_log || 'No reading log yet';

    document.getElementById('analysisLog').textContent =
      s.analysis_log || 'No analysis log yet';

    document.getElementById('eventLog').textContent =
      s.event_log || 'No event log yet';
  }
  catch (e)
  {
  }
...
setInterval(loadLogs, 10000);
  ...
  static void handleStatus()
  {
    ...
    json += "\"relay_on\":";
    json += load_relay::isOn()
                ? "true"
                : "false";
    json += ",";

    json += "\"relay_requested\":";
    json += load_relay::getRequestedState()
                ? "true"
                : "false";
    json += ",";
    ...
    json += "\"sd_ready\":";
    json += storage::isSdReady()
                ? "true"
                : "false";
    json += ",";

    json += "\"internal_ready\":";
    json += storage::isInternalReady()
                ? "true"
                : "false";
    json += ",";
    ...
    json += "\"reading_log_size\":";
    json += String(
        storage::getFileSize(
            storage::getLiquidLogFileName()));
    json += ",";
  ...
  static void handleSaveSettings()
  {
    ...
    float fullPercent =
        server.arg("full").toFloat();

    float lowPercent =
        server.arg("low").toFloat();

    int mode =
        server.arg("mode").toInt();

    if (
        fullPercent <= lowPercent ||
        fullPercent > 100.0f ||
        lowPercent < 0.0f ||
        mode < 0 ||
        mode > 2)
    ...
    device_manager::setRelayLatchMode(
        static_cast<
            device_manager::RelayLatchMode>(mode));
  ...
  static void handlereadingDownload()
  {
    streamCsv(
        storage::getLiquidLogFileName());
  }
  ...
  void begin()
  {
    ...
    server.on(
        "/download/reading_log.csv",
        HTTP_GET,
        handlereadingDownload);
